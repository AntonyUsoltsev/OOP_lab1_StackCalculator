...
public class Multiply implements Command {
    ...
    public void action(String[] commandArgs, Calculator.Parameters parameters) {
        if (commandArgs.length != 1) {
            System.err.println("Unknown args in Multiply command.");
            LOGGER.log(Level.SEVERE, "Unknown args in Multiply command.\n");
            //throw new IllegalArgumentException("Invalid arguments for PLUS command.");
        } else if (parameters.getStack().size() < 2) {
            System.err.println("Stack must have at least two elements to perform the Multiply operation.");
            LOGGER.log(Level.SEVERE, "Stack must have at least two elements to perform the Multiply operation.\n");
